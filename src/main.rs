...
fn run(sharp: Process, gas:u64, mem:u64, debug:bool) -> Process {
    ...
    loop {
        ...
        let blockret = {
            ...
            if state.header.status != Stati::NOR as u64 {//&& state.header.status != REC
        ...
        if debug {
            println!("INSTR: {} {:?}", IS::string_from_int(instr as u8), state.header.ip);
        }
        if jump_back > -2 {
            //pass
        } else if ie == IS::RUN {
            ...
            if valid_area(area, &states[statelen]) {
                ...
                if states[statelen].header.rec > 0 && states[statelen].memory[area as usize][0] == Stati::NOR as u64 {
            ...
            let stackdiff : i64 = ((states[(states.len()-1) as usize].stack.len() as i64)-(stacklen as i64)) as i64;
...
fn main() {
    ...
    let flat = vec![0, 0, 10000000, 100000000, 0, 713, 0, 0, 0, 6, 0, 19, 6, 0, 6, 1, 21, 6, 425, 4, 6, 0, 6, 2, 21, 6, 0, 8, 16, 6, 2, 24, 6, 0, 8, 17, 18, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 8, 16, 6, 2, 24, 18, 6, 42, 6, 0, 8, 8, 17, 6, 1, 24, 17, 2, 6, 0, 8, 16, 6, 1, 24, 17, 6, 0, 8, 16, 6, 2, 24, 17, 6, 0, 8, 16, 6, 0, 8, 17, 24, 22, 6, 0, 8, 32, 18, 4, 6, 0, 6, 3, 21, 6, 0, 8, 16, 6, 2, 24, 6, 0, 8, 17, 18, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 8, 16, 6, 3, 24, 18, 6, 0, 8, 8, 8, 17, 32, 18, 6, 0, 8, 8, 17, 17, 6, 0, 8, 8, 17, 6, 2, 24, 6, 1, 23, 17, 24, 6, 216, 5, 6, 0, 8, 8, 8, 17, 6, 2, 24, 17, 6, 0, 8, 8, 17, 17, 23, 17, 6, 0, 6, 1, 21, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 192, 6, 11, 4, 6, 0, 6, 1, 22, 6, 0, 8, 8, 17, 17, 6, 1, 23, 6, 0, 8, 8, 17, 32, 18, 6, 133, 4, 6, 0, 8, 16, 6, 1, 24, 17, 6, 0, 8, 16, 6, 2, 24, 17, 6, 0, 8, 16, 6, 0, 8, 17, 24, 22, 6, 0, 8, 32, 18, 4, 6, 0, 6, 2, 21, 6, 0, 8, 16, 6, 2, 24, 6, 0, 8, 17, 18, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 8, 16, 6, 2, 24, 18, 6, 0, 8, 8, 17, 6, 2, 24, 17, 6, 0, 8, 8, 17, 6, 2, 24, 6, 1, 23, 17, 6, 0, 6, 2, 21, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 16, 6, 2, 24, 32, 18, 6, 332, 6, 90, 4, 6, 0, 6, 2, 22, 6, 0, 6, 1, 21, 6, 0, 8, 16, 6, 1, 24, 6, 10, 18, 6, 0, 16, 6, 1, 24, 6, 1, 6, 0, 6, 2, 21, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 16, 6, 2, 24, 32, 18, 6, 388, 6, 90, 4, 6, 0, 6, 2, 22, 6, 0, 8, 16, 6, 1, 24, 17, 6, 0, 8, 16, 6, 2, 24, 17, 6, 0, 8, 16, 6, 0, 8, 17, 24, 22, 6, 0, 8, 32, 18, 4, 6, 0, 6, 5, 21, 6, 0, 8, 16, 6, 2, 24, 6, 0, 8, 17, 18, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 8, 16, 6, 5, 24, 18, 6, 0, 6, 13, 21, 6, 0, 8, 16, 6, 13, 24, 6, 104, 18, 6, 0, 8, 16, 6, 12, 24, 6, 101, 18, 6, 0, 8, 16, 6, 11, 24, 6, 108, 18, 6, 0, 8, 16, 6, 10, 24, 6, 108, 18, 6, 0, 8, 16, 6, 9, 24, 6, 111, 18, 6, 0, 8, 16, 6, 8, 24, 6, 44, 18, 6, 0, 8, 16, 6, 7, 24, 6, 32, 18, 6, 0, 8, 16, 6, 6, 24, 6, 119, 18, 6, 0, 8, 16, 6, 5, 24, 6, 111, 18, 6, 0, 8, 16, 6, 4, 24, 6, 114, 18, 6, 0, 8, 16, 6, 3, 24, 6, 108, 18, 6, 0, 8, 16, 6, 2, 24, 6, 100, 18, 6, 0, 8, 16, 6, 1, 24, 6, 33, 18, 6, 0, 16, 6, 13, 24, 6, 13, 6, 0, 8, 8, 17, 6, 1, 23, 6, 1, 23, 32, 18, 6, 0, 8, 8, 17, 6, 1, 23, 32, 18, 6, 0, 8, 8, 17, 6, 1, 23, 17, 6, 0, 8, 8, 17, 6, 1, 23, 6, 1, 23, 17, 6, 0, 6, 2, 21, 6, 0, 8, 16, 6, 1, 24, 32, 18, 6, 0, 8, 16, 6, 2, 24, 32, 18, 6, 675, 6, 248, 4, 6, 0, 6, 2, 22, 6, 0, 8, 8, 16, 6, 1, 24, 17, 6, 0, 8, 16, 6, 2, 24, 17, 6, 0, 8, 16, 6, 0, 8, 17, 24, 22, 6, 0, 8, 32, 18, 0];
    //println!("Start");
    let debug = false;
    ...
    loop {
        ...
        //print!("{}", stacklen);
        if sharp.header.status == (Stati::YLD as u64) && stacklen >= 2 && sharp.stack[stacklen-2] == 42 {
